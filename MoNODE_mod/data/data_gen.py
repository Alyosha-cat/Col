...
def load_ais_data(data_path, params, flag, task='ais'):
	DATA_ROOT = "AIS/"
	fnames = os.listdir(DATA_ROOT)
	print(fnames)
	for name in fnames:
		if name.endswith('data.pickle'):
			with open(os.path.join(DATA_ROOT, name), 'rb') as f:
				X = pickle.load(f)
	array = np.array(X[['longitude', 'latitude']])
	X = torch.tensor(array.reshape([-1, 100, 2]))

	if flag=='train':
		torch.save(X[:-13],os.path.join(DATA_ROOT,'ais/ais-tr-data.pkl'))
	elif flag=='valid':
		torch.save(X[-13:-8],os.path.join(DATA_ROOT,'ais/ais-vl-data.pkl'))
	elif flag=='test':
		torch.save(X[-8:],os.path.join(DATA_ROOT,'ais/ais-te-data.pkl'))