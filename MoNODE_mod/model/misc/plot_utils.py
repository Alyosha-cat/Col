...
class Plotter:
    def __init__(self, root, task_name):
        ...
        if self.task_name == 'lv' or self.task_name=='ais':
            self.plot_fit_fnc = plot_2d
        if 'mocap' in self.task_name:
            self.plot_fit_fnc = plot_mocap
...
def plot_mocap(X, Xrec, show=False, fname='predictions.png', N=None, D=None):
    '''
         X   - [N,T,d] 
        Xrec - [L,N,Ttest,d]
    '''
    ...
    Xrecnp = Xrec.detach().cpu().numpy()
    for n in range(N):
        fig, axes = plt.subplots(10, 5, figsize=(40,20), squeeze=False)
        for i in range(10):
            for j in range(5):
                axes[i,j].plot(Xnp[n,:,5*i+j], '-', color='tab:blue', lw=2)
                axes[i,j].plot(Xrecnp[:,n,:,5*i+j].T, '-', color='tab:gray', lw=0.7, alpha=0.5)
        if show:
            plt.show()
        else:
            plt.tight_layout()
            plt.savefig(f'{fname}-{n}.png')
            plt.close()
